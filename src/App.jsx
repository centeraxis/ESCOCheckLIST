                ...
                <div className="flex bg-white rounded-lg shadow p-1 mb-6 sticky top-28 z-10 overflow-x-auto">
                    {checklistData.map(tab => (
                        <button
                            ...
                            className={`flex-1 py-2 px-3 text-sm font-medium rounded-md whitespace-nowrap transition-colors ${activeTab === tab.id
                ...
                <div className="space-y-6">
                    {checklistData.find(d => d.id === activeTab)?.groups.map((group) => {
                        ...
                        return (
                            <div
                                key={group.id}
                                className={`bg-white rounded-xl shadow-sm border transition-all duration-200 ${isComplete ? 'border-green-200 shadow-green-50' : 'border-gray-200'
                                    }`}
                            >
                                {/* Group Header */}
                                <div
                                ...
                                {!isCollapsed && (
                                    <div className="border-t border-gray-100 divide-y divide-gray-100">
                                        {group.items.map((item) => (
                                            <label
                                                key={item.id}
                                                className={`flex items-start p-4 cursor-pointer hover:bg-gray-50 transition-colors ${checkedItems[item.id] ? 'bg-blue-50/30' : ''
                                                    }`}
                                            >
                                                ...
                                                <span className={`ml-3 text-sm leading-relaxed select-none ${checkedItems[item.id] ? 'text-gray-400 line-through' : 'text-gray-700'
                                                    }`}>
                                                    {item.label}
                                                </span>
                                            </label>
